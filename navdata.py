...
def parseData( data, drone, verbose=False ):
    ...
    if frameId == 0x7F:
        ...
        if commandProject == 0:
            ...
            if (commandClass, commandId) == (4,4):
                ...
                drone.speed = (speedX, speedY, speedZ)
                drone.position = drone.position[0]+POSITION_TIME_DELTA*speedX, drone.position[1]+POSITION_TIME_DELTA*speedY, drone.position[2]+POSITION_TIME_DELTA*speedZ
                if verbose:
                    print "Speed", speedX, speedY, speedZ
# Angle
            elif (commandClass, commandId) == (4,6):
                roll, pitch, yaw = struct.unpack("fff", data[11:11+3*4])
                drone.angle=(roll, pitch, yaw)
                if verbose:
                    print "Angle", roll, pitch, yaw
            
            elif (commandClass, commandId) == (4,8):
                drone.altitude = struct.unpack("d", data[11:11+8])[0]            
                if verbose:
                    print "Altitude", drone.altitude
                ...
                try:
                    ...
                    drone.moveByEnd = (dX, dY, dZ, dPsi, Event)
                    if verbose:
...
if __name__ == "__main__":
    ...
    while data:
        data = parseData( data, drone, verbose=True )
